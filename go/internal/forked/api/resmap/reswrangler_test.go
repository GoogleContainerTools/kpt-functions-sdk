...
func doAppend(t *testing.T, w resmap.ResMap, r *resource.Resource) {
	err := w.Append(r)
	if err != nil {
		t.Fatalf("append error: %v", err)
	}
}
func doRemove(t *testing.T, w resmap.ResMap, id resid.ResId) {
	err := w.Remove(id)
	if err != nil {
...
func makeMap2(b types.GenerationBehavior) resmap.ResMap {
	return rmF.FromResource(rf.FromMapAndOption(
		map[string]interface{}{
			"apiVersion": "apps/v1",
			"kind":       "ConfigMap",
