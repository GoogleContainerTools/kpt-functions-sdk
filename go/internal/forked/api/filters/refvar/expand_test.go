...
func TestPrimitiveReplacer(t *testing.T) {
	...
	f := refvar.MakePrimitiveReplacer(
		varCounts,
...
func TestMapReference(t *testing.T) {
	...
	for _, env := range envs {
		varMap[env.Name] = refvar.DoReplacements(
			fmt.Sprintf("%v", env.Value),
			refvar.MakePrimitiveReplacer(varCounts, varMap))
	}
...
func TestMapping(t *testing.T) {
	...
	for _, tc := range cases {
		...
		expanded := refvar.DoReplacements(
			fmt.Sprintf("%v", tc.input),
			refvar.MakePrimitiveReplacer(counts, map[string]interface{}{
				"VAR_A":     "A",
				"VAR_B":     "B",
				"VAR_C":     "C",
				"VAR_REF":   "$(VAR_A)",
