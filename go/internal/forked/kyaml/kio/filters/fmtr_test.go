...
import (
	...
	"github.com/GoogleContainerTools/kpt-functions-sdk/krmfn/internal/forked/kyaml/kio/filters/testyaml"
...
`
	...
	s, err := filters.FormatInput(strings.NewReader(y))
	assert.NoError(t, err)
...
func TestFormatInput_resources(t *testing.T) {
	...
	s, err := filters.FormatInput(input)
	assert.NoError(t, err)
...
`
	...
	b, err := filters.FormatInput(strings.NewReader(y))
	assert.NoError(t, err)
...
`
	...
	_, err := filters.FormatInput(strings.NewReader(y))
...
func TestFormatFileOrDirectory_yamlExtFile(t *testing.T) {
	...
	// format the file
	err = filters.FormatFileOrDirectory(f.Name())
	if !assert.NoError(t, err) {
		return
	}
...
func TestFormatFileOrDirectory_directory(t *testing.T) {
	...
	err = filters.FormatFileOrDirectory(d)
	assert.NoError(t, err)
