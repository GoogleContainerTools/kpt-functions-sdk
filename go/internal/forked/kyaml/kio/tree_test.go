...
`
	...
	err := kio.Pipeline{
		...
		Outputs: []kio.Writer{kio.TreeWriter{Writer: out, Structure: kio.TreeStructurePackage}},
	}.Execute()
...
func TestPrinter_metaError(t *testing.T) {
	out := &bytes.Buffer{}
	err := kio.TreeWriter{Writer: out}.Write([]*yaml.RNode{{}})
	if !assert.NoError(t, err) {
		t.FailNow()
	}
	if !assert.Equal(t, `
...
`
	...
	err := kio.Pipeline{
		Inputs:  []kio.Reader{&kio.ByteReader{Reader: bytes.NewBufferString(in)}},
		Outputs: []kio.Writer{kio.TreeWriter{Writer: out, Structure: kio.TreeStructureGraph}},
	}.Execute()
	if !assert.NoError(t, err) {
		t.FailNow()
	}
...
`
	...
	err := kio.Pipeline{
		Inputs:  []kio.Reader{&kio.ByteReader{Reader: bytes.NewBufferString(in)}},
		Outputs: []kio.Writer{kio.TreeWriter{Writer: out}}, // Structure unspecified
	}.Execute()
	if !assert.NoError(t, err) {
		t.FailNow()
	}
...
`
	...
	err := kio.Pipeline{
		Inputs:  []kio.Reader{&kio.ByteReader{Reader: bytes.NewBufferString(in)}},
		Outputs: []kio.Writer{kio.TreeWriter{Writer: out}},
	}.Execute()
	assert.Error(t, err)
