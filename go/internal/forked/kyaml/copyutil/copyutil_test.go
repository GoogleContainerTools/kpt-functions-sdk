...
import (
	...
	"github.com/GoogleContainerTools/kpt-functions-sdk/krmfn/internal/forked/kyaml/copyutil"
)
...
func TestSyncFile(t *testing.T) {
	...
	assert.NoError(t, err)
	err = copyutil.SyncFile(f1Name, f2Name)
	assert.NoError(t, err)
...
metadata:
  ...
  namespace: "some_project" # {"$ref":"#/definitions/io.k8s.cli.setters.gcloud.core.project"}`
	...
	assert.Contains(t, copyutil.PrettyFileDiff(s1, s2), expectedLine1)
	assert.Contains(t, copyutil.PrettyFileDiff(s1, s2), expectedLine2)
}
...
func TestCopyDir(t *testing.T) {
	...
	err = copyutil.CopyDir(s, d)
	assert.NoError(t, err)

	diff, err := copyutil.Diff(d, v)
	assert.NoError(t, err)
	assert.Empty(t, diff.List())
}
...
func TestIsDotGitFolder(t *testing.T) {
	...
	for i := range testCases {
		...
		t.Run(test.name, func(t *testing.T) {
			assert.Equal(t, test.isDotGitFolder, copyutil.IsDotGitFolder(test.path))
		})
	}
}
