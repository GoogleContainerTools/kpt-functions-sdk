...
import (
	...
	yaml2 "github.com/GoogleContainerTools/kpt-functions-sdk/krmfn/internal/forked/kyaml/internal/forked/github.com/go-yaml/yaml"
)
...
var nodeTests = []struct {
...
}{
	{
		...
		yaml2.Node{
			...
			Column: 1,
			Content: []*yaml2.Node{{
				Kind:   yaml2.MappingNode,
				Style:  yaml2.TaggedStyle | yaml2.FlowStyle,
				Tag:    "!tag:something",
				Line:   1,
				Column: 1,
			}},
...
func (s *S) TestNodeRoundtrip(c *C) {
	...
	for i, item := range nodeTests {
		...
		if decode {
			var node yaml2.Node
			err := yaml2.Unmarshal([]byte(testYaml), &node)
			c.Assert(err, IsNil)
			if strings.Contains(item.yaml, "#") {
				var buf bytes.Buffer
...
func deepCopyNode(node *yaml2.Node, cache map[*yaml2.Node]*yaml2.Node) *yaml2.Node {
	if n, ok := cache[node]; ok {
		return n
	}
	if cache == nil {
		cache = make(map[*yaml2.Node]*yaml2.Node)
	}
	copy := *node
	cache[node] = &copy
	copy.Content = nil
...
var savedNodes = make(map[string]*yaml2.Node)

func saveNode(name string, node *yaml2.Node) *yaml2.Node {
	savedNodes[name] = node
	return node
}

func peekNode(name string) *yaml2.Node {
	return savedNodes[name]
}

func dropNode(name string) *yaml2.Node {
...
func (s *S) TestSetString(c *C) {
	...
	for i, item := range setStringTests {
		...
		var doc yaml2.Node
		err = yaml2.Unmarshal([]byte(item.yaml), &doc)
		c.Assert(err, IsNil)
...
func (s *S) TestNodeOmitEmpty(c *C) {
	...
	v.B.Line = 1
	_, err = yaml2.Marshal(&v)
	c.Assert(err, ErrorMatches, "yaml: cannot encode node with unknown kind 0")
}

func fprintComments(out io.Writer, node *yaml2.Node, indent string) {
	switch node.Kind {
	case yaml2.ScalarNode:
		fmt.Fprintf(out, "%s<%s> ", indent, node.Value)
		fprintCommentSet(out, node)
		fmt.Fprintf(out, "\n")
	case yaml2.DocumentNode:
...
func fprintCommentSet(out io.Writer, node *yaml2.Node) {
